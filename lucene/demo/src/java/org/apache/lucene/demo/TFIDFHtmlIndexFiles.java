...
public class TFIDFHtmlIndexFiles {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      // CMPT456Analyzer stopWordsAnalyzer = new CMPT456Analyzer();

      Analyzer analyzer = new StandardAnalyzer();
      Similarity similarity = new CMPT456Similarity();
  ...
  static void indexDoc(IndexWriter writer, Path file, long lastModified) throws IOException {
    try (InputStream stream = Files.newInputStream(file)) {
      ...
      // ref: https://www.codota.com/code/java/classes/org.apache.lucene.benchmark.byTask.feeds.DocData
      // Add the contents of the file to a field named "contents".  Specify a Reader,
      // so that the text of the file is tokenized and indexed, but not stored.
      // Note that FileReader expects the file to be in UTF-8 encoding.
      // If that's not the case searching for special characters will fail.
      //doc.add(new TextField("contents", new BufferedReader(new InputStreamReader(stream, StandardCharsets.UTF_8))));
      DemoHTMLParser dHTMLparser = new DemoHTMLParser();
      DocData docData = new DocData();
      docData = dHTMLparser.parse(docData, "", null, new BufferedReader(new InputStreamReader(stream, StandardCharsets.UTF_8)), null);
